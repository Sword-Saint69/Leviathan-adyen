    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt == max_retries:
                print(f"[shopify] graphql request timeout: {type(e).__name__}: {repr(e)}")
                return None, str(e), False
            await asyncio.sleep(1)
        except Exception as e:
            if attempt == max_retries:
                print(f"[shopify] graphql request error: {type(e).__name__}: {repr(e)}")
                return None, str(e), False
            await asyncio.sleep(1)
...
async def fetch_products(domain, proxy_str=None):
    try:
        ...
        print(f"[shopify] fetch_products domain={domain} proxy={proxy}")
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            async with session.get(f"{domain}/products.json?limit=250", proxy=proxy, timeout=30) as resp:
                if resp.status != 200:
                    return False, f"<b>Site Error! Status: {resp.status}</b>"
                try:
                    data = await resp.json()
                except Exception:
                    return False, "<b>Invalid JSON!</b>"
        ...
        print(f"[shopify] fetch_products aiohttp.ClientError: {type(e).__name__}: {repr(e)}")
        return False, "Proxy Error"
    except asyncio.TimeoutError as e:
        print(f"[shopify] fetch_products timeout: {type(e).__name__}: {repr(e)}")
        return False, "Proxy Error"
    except Exception as e:
        print(f"[shopify] fetch_products exception: {type(e).__name__}: {repr(e)}")
        return False, "Proxy Error"
...
async def process_card(cc, mes, ano, cvv, site_url, variant_id=None, proxy_str=None):
    ...
    try:
        ...
        print(f"[shopify] process_card proxy error: {type(e).__name__}: {repr(e)}")
        return False, "Proxy Error", gateway, total_price, currency
    except Exception as e:
        print(f"[shopify] process_card exception: {type(e).__name__}: {repr(e)}")
...
def save_hit_to_file(cc_string: str, status: str, gateway: str = "", price: str = ""):
    try:
        ...
        print(f"[shopify] save_hit_to_file error: {exc}")
# ── Live-card response keywords (mirrors bot12.py logic) ────────────────────
_LIVE_KEYWORDS = (
...
def shopify_checker():
    try:
        ...
        if hit_status:
            save_hit_to_db(cc_string, hit_status, gateway, str(price))
